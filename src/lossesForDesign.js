...
const lossesForDesign = function ({
  ...
  let percentage_stray_loss_for_design // %

  if (power_rating <= 100) {
    i2r_loss_for_design = specified_copper_loss - power_rating
    percentage_stray_loss_for_design =
      (100 * power_rating) / specified_copper_loss
    if (ht_conductor_material === ALUMINIUM) {
      percentage_stray_loss_for_design *= 1.3
      i2r_loss_for_design = specified_copper_loss - 1.3 * power_rating
    }
  } else if (power_rating <= 250) {
    percentage_stray_loss_for_design = 8
    if (ht_conductor_material === ALUMINIUM) {
      percentage_stray_loss_for_design = 10
      if (power_rating < 200) {
        percentage_stray_loss_for_design = 9
      }
    }
    i2r_loss_for_design =
